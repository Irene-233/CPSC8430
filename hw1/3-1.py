...
print("- Validation-sett{}".format(len(data.validation.labels)))
#index = [i for i in range(len(data.train.labels))] 
random.shuffle(data.train.labels)
#data.train.labels = data.train.labels[index]
...
grads = tf.gradients(loss1, weights_fc_out)[0]
print(grads)
...
def print_test_accuracy():
    ...
    while i < num_test:
        ...
        feed_dict = {x: images,
                     y_true: labels}
        cls_pred[i:j],test_loss = session.run([y1_pred_cls,loss1], feed_dict=feed_dict)
        i = j
    losstest_list.append(test_loss)
...
def optimize(num_iterations):
    ...
    for i in range(total_iterations,
        ...
        if i % 1 == 0:
            ...
            grads_vals = session.run([grads], feed_dict=feed_dict_train)
...
plt.plot(loss1_list,'b-',losstest_list,'r-')
